...
public sealed class ExecutionEngineTests
{
    ...
    private static ExecutionEngine CreateEngine(FakeRepository repository, FakeApiClient api) => new(repository, repository, new FixedRequestBuilder(), api, new JsonValidationEngine(), new FrameworkSettings { MaxRetries = 1, Environments = new(StringComparer.OrdinalIgnoreCase) { ["qa"] = new("qa", "https://api.example.test", 100, null, new Dictionary<string, string>()) } }, new NullProgressSink());
    private static TestCaseDefinition TestCase(string method = "GET") => new("TC-1", "Smoke", "API", "/health", method, "health", true, 1, 200, new Dictionary<string, string>(), new Dictionary<string, string>(), new Dictionary<string, string>(), null, []);

    private sealed class FixedRequestBuilder : IRequestBuilder { public ApiRequest Build(TestCaseDefinition testCase, EnvironmentConfiguration environment, string correlationId) => new(testCase.Method, new Uri("https://api.example.test/health"), new Dictionary<string, string>(), testCase.Payload, 100); }
    private sealed class NullProgressSink : IExecutionProgressSink { public void Complete(ExecutionSummary summary) { } public void Report(TestExecutionResult result) { } }
    private sealed class FakeApiClient(params object[] outcomes) : IApiClient
    {
        ...
        public Task<ApiResponse> SendAsync(ApiRequest request, CancellationToken cancellationToken) { Calls++; var outcome = _outcomes.Dequeue(); return outcome is Exception error ? Task.FromException<ApiResponse>(error) : Task.FromResult((ApiResponse)outcome); }
    ...
    private sealed class FakeRepository(IReadOnlyList<TestCaseDefinition> definitions) : ITestCaseProvider, IResultRepository
    {
        ...
        public Task PersistAsync(TestExecutionResult result, CancellationToken cancellationToken) { if (FailPersist) throw new InvalidOperationException("SharePoint unavailable"); Persisted.Add(result); return Task.CompletedTask; }
    }
}
